	...
	bool bEnableCache)
{
	...
	if (!pSignAsset->m_strProvisionData.empty()) {
		if (!WriteFileV(pSignAsset->m_strProvisionData, "%s/embedded.mobileprovision", m_strAppFolder.c_str())) { // embedded.mobileprovision
	...
	if (!arrDyLibFiles.empty()) { // inject dylib
		for (size_t i = 0; i < arrDyLibFiles.size(); i++) {
			...
			if (!strDyLibData.empty()) {
				...
				if (WriteFileV(strDyLibData, "%s/%s", m_strAppFolder.c_str(), strFileName.c_str())) {
					string strDyLibPath;
					StringFormatV(strDyLibPath, "@executable_path/%s", strFileName.c_str());
					m_arrDyLibPaths.push_back(strDyLibPath);
				}
			}
		}
	}
